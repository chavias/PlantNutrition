...
def plot_excess_T(df_transposed, field='excess'):
    # Define parameters for the plot
    days = df_transposed.index
    num_excess_values = len(df_transposed[field].iloc[0])
    x = np.arange(num_excess_values)  # the label locations
    width = 1/len(days)  # the width of the bars

    # Plot each day's excess values with unique x-axis positions
    plt.figure(figsize=(40, 6))

    for i, day in enumerate(days):
        plt.bar(x + i*width, df_transposed.loc[day, field], width=width, label=day)

    # Adding gridlines
    plt.grid(axis='x', which='both', linestyle='--', linewidth=0.7, color='grey')  # Vertical gridlines between excess values

    # Labeling
    plt.xlabel('Excess Index')
    plt.ylabel('Excess Value')
    plt.title('Excess Values for Different Days')
    plt.xticks(x + width, labels=[f'Excess {i}' for i in range(num_excess_values)])
    plt.legend()
    plt.tight_layout()
    # Show plot
    plt.show()
...
def plot_excess(df_transposed):
    # Define parameters for the plot
    excess_labels = [f'Excess {i}' for i in range(len(df_transposed['excess'].iloc[0]))]
    num_days = len(df_transposed.index)
    x = np.arange(num_days)  # label locations for days
    width = 0.1  # Adjust the width of the bars to fit all excess groups

    # Plot each excess group across the days
    plt.figure(figsize=(10, 6))

    for i, label in enumerate(excess_labels):
        excess_values = [df_transposed.loc[day, 'excess'][i] for day in df_transposed.index]
        plt.bar(x + i*width, excess_values, width=width, label=label)

    # Adding gridlines
    # plt.grid(axis='x', which='both', linestyle='--', linewidth=0.7, color='grey')  # Vertical gridlines between days
    ...
    # Adjust x-ticks to ensure they align well with the bars
    
    plt.xticks(x, labels=[i+1 for i in range(num_days)])
    # plt.xticks(x + width, labels=df_transposed.index)
...
def plot_excess_stacked_interactive(df_transposed):
